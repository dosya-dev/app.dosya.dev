...
export function AudioPlayer({ file, files, rawUrl, version, onNavigate }: Props) {
  ...
  // A WMA everywhere, or an OGG on a browser without the decoder, fails
  // silently: the element just never fires canplay and every control is a
  // no-op. Naming the problem beats a transport that ignores clicks. The
  // failure is remembered per-url, so navigating to the next track clears
  // it without a reset effect.
      ...
      <div className="flex flex-col px-8 pt-7 pb-6">
        ...
        {decodeFailed ? (
          <div className="mt-[22px] flex h-[78px] items-center justify-center rounded-lg border border-dashed px-4 text-center text-xs text-muted-foreground">
            This browser can't decode this audio format. Download the file to play it in another app.
          </div>
