...
/** Root segment of the duplicates query key, for workspace-wide invalidation. */
...
export function fetchDuplicates(workspaceId: string): Promise<DuplicatesResponse> {
  return api<DuplicatesResponse>(`/api/duplicates?workspace_id=${encodeURIComponent(workspaceId)}`);
}
