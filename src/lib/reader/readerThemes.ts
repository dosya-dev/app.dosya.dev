 ...
 * looks the same on all three clients.
 *
 * NOTE ON DEPLOYING THIS DIRECTORY: apps/web reaches production through the
 * dosya-dev/app.dosya.dev mirror, and sync-web only runs when the whole gate
 * is green AND apps/web itself changed in that push. A red gate elsewhere in
 * the monorepo strands web changes indefinitely - this reader sat unshipped
 * for a day behind an unrelated api-integration failure.
 *
 ...
 *
 * The app states its palette in CSS custom properties as oklch(), which nothing
 * outside a browser can interpret - so rather than parse them, this paints a
