 ...
 * ImportProgressCard's mount effect triggers the first refresh(), and
 * refresh() itself owns a self-starting/self-stopping poll timer (see
 * CLOUD_IMPORT_POLL_MS in stores/cloud-imports.ts) that keeps calling it
 * while any job stays active. This hook only reacts to that array changing:
 * it diffs the previous active-job-id set against the current one on every
 * change. An id that drops out of "active" is a completion edge; an id that
 * stays active (an ordinary progress tick - counts changed, status didn't) or was never
